...
class DictionaryController {
  static async getAllWords(req, res) {
    try {
      ...
      const words = await DictionaryService.getAllWords(user.id);

      if (words.length === 0) return res.json(formatResponse(200, 'Слов нет'));
  ...
  static async createWord(req, res) {
    try {
      ...
      const newWord = await DictionaryService.createWord({
        word,
        description,
        tag,
        userId,
      });
      return res.status(201).json(formatResponse(201, 'Слово добавлено', newWord));
    } catch (error) {
      console.log(error);
      return res.status(500).json(formatResponse(500, 'Server Error'));
    }
    ...
    try {
      ...
      if (!isValid)
        return res
          .status(400)
          .json(formatResponse(400, 'Валидация не прошла', null, err));
      const updatedWord = await DictionaryService.updateWord(id, {
        word,
        description,
        tag,
        userId,
      });
      return res.status(200).json(formatResponse(200, 'Слово изменено', updatedWord));
    } catch (error) {
      console.log(error);
      return res.status(500).json(formatResponse(500, 'Server Error'));
    }
